...
const RawLocationSchema = z.object({
  ...
  channelSubscriberCount: z.number().int().nonnegative().optional(),
  youtubeChannelId: z.string().min(1).optional(),
  image: z.string().url(),
  highResImage: z.string().url().optional(),
...
const LocationItemSchema = RawLocationSchema.transform((raw) => {
  ...
  return {
    ...
    channelSubscriberCount: raw.channelSubscriberCount ?? null,
    youtubeChannelId: raw.youtubeChannelId ?? null,
    image: raw.image,
    highResImage: raw.highResImage ?? null,
    lat,
    lng,
